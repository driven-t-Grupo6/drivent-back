...
export async function getHotelsByRoomId(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  const { userId } = req;
  const { roomId } = req.params;

  try {
    const hotel = await hotelsService.getHotelsByRoomId(userId, Number(roomId));
